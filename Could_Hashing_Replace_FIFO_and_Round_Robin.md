...
# 6. Open Problems and Research Vectors - Security Model
...
> The most interesting claim is : a hash scheduler with a well-designed composite key is not just a scheduling algorithm — it is a declarative scheduling *policy language*. Change the key composition, change the policy. No code changes required.
...
The kernel is one of the few places left where a single elegant idea can touch every process on a machine. Hash scheduling might be one such idea — waiting for someone to run the experiment.
